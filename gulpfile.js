...
gulp.task('css', ['clean'], function() {
    return gulp.src('app.less')
        ...
        .pipe(rename('app-v2.full.min.css'))
        .pipe(gulp.dest('dist/'));
});
...
gulp.task('js', ['clean'], function() {
    ...
    return combined.done()
        .pipe(concat('app-v2.full.min.js'))
        .pipe(ngmin())
        .pipe(uglify())
        .pipe(gulp.dest('dist/'));
...
gulp.task('indexHtml', ['clean'], function() {
    return gulp.src('index.html')
        .pipe(gCheerio(function ($) {
            ...
            $('link').remove();
            $('body').append('<script src="app-v2.full.min.js"></script>');
            $('head').append('<link rel="stylesheet" href="app-v2.full.min.css">');
        }))
        .pipe(htmlmin(htmlminOptions))
        .pipe(gulp.dest('dist/'));
});
...
gulp.task('phpmove', ['clean'], function(){
    return gulp.src('*.php')
        .pipe(gulp.dest('dist/'));
});
